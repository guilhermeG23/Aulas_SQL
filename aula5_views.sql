...
#A-)
create view tabela_vw as select produto.Nome as produto, produto.Validade, marca.Nome as marca from produto inner join marca on produto.MarcaId = marca.Id;

#B-)
select * from saida_vw where year(Validade) = 2018;

#C-)
create view tabela_vw_1 as select marca.Nome as marca, produto.Nome as produto, financeiro.Qtd * financeiro.VL_Compra as Valor from marca inner join produto on marca.Id = produto.MarcaID inner join financeiro on produto.Codigo = financeiro.ProdCod;

#D-) Funcionou
select * from tabela_vw_1;

#E-)
create view tabela_vw_2 as select marca.Nome as marca, produto.Nome as produto, financeiro.Qtd as quantidade, financeiro.Qtd * ((financeiro.VL_Compra) / 100) * 60 as lucro, (financeiro.VL_Compra / 100) * 60 from marca inner join produto on marca.Id = produto.MarcaId inner join financeiro on produto.Codigo = financeiro.ProdCod;
#F-)
select * from tabela_vw_2;

#G-)
delete view tabela_vw;

#H-)
show tables;

#i-)
select sum(lucro) from tabela_vw_2;